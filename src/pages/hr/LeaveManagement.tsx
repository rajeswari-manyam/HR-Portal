...
export default function LeaveManagement() {
  ...
  const columns = [
    ...
    },
    {
      header: 'Duration',
      render: (r: LeaveRequest) => (
        <span className="text-sm text-slate-600">{formatDate(r.startDate)} – {formatDate(r.endDate)}</span>
      ),
    },
    {
      header: 'Days',
      render: (r: LeaveRequest) => (
        <span className="font-bold text-slate-900">{r.days} day{r.days > 1 ? 's' : ''}</span>
      ),
    },
    {
      header: 'Reason',
    ...
    {
      header: 'Applied',
      render: (r: LeaveRequest) => <span className="text-sm text-slate-500">{formatDate(r.appliedOn)}</span>,
    },
    {
      header: 'Status',
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
            {(['', 'pending', 'approved', 'rejected'] as const).map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all capitalize ${statusFilter === s ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}>
                {s || 'All'}
              </button>
            ))}
          </div>
      ...
      <Table columns={columns} data={filtered} rowsPerPage={6} emptyMessage="No leave requests found" />
          ...
          <div className="space-y-4">
            ...
            {selected.status === 'pending' && (
              <div className="flex gap-3">
                <button onClick={() => { updateStatus(selected.id, 'rejected'); setViewModal(false); }}
                  className="btn-danger flex-1 justify-center">Reject</button>
                <button onClick={() => { updateStatus(selected.id, 'approved'); setViewModal(false); }}
                  className="btn-success flex-1 justify-center">Approve</button>
              </div>
            )}
          </div>
        </Modal>
